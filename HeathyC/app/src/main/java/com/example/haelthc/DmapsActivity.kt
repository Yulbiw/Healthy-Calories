...
import kotlinx.android.synthetic.main.activity_dmaps.*

class DmapsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etplace = findViewById(R.id.lolo_d)
        btSubmit = findViewById(R.id.bt_location_search_d)
        tvaddress = findViewById(R.id.tv_address_d)
        ...
        recode_map_button_d.setOnClickListener {
    ...
    private inner class GeoHandler : Handler(), OnMapReadyCallback {
        override fun handleMessage(msg: Message) {
            ...
            mapFragment = supportFragmentManager.findFragmentById(R.id.map_d) as SupportMapFragment?
            mapFragment?.getMapAsync(this)
        }

        override fun onMapReady(googleMap: GoogleMap?) {
            mMap = googleMap
            val mGPS: String = (findViewById(R.id.tv_address_d) as TextView).text.toString()
            ...
            if (list != null) {
                ...
                google_location_d.append("  $address")
//                tv_address.visibility = View.GONE
            } else {
                google_location_d.append("Null")
                tv_address_d.visibility = View.GONE
            }
            val recDDate: Bundle? = intent.extras
            val dateka = recDDate!!.getString("dateD")
            var userEmail = "";
                ...
                .addValueEventListener(object :
                    ...
                    override fun onDataChange(p0: DataSnapshot) {
                        for (snapshot in p0.children) {
                            ...
                            val dinbase = snapshot.value as Map<String, Any>
                            val date = dinbase["date"].toString()
                            val mail = dinbase["userEmail"].toString()
                            if (dateka == date && mail == userEmail) {
                                var map = mutableMapOf<String, Any>()
                                map["location"] = google_location_d.text.toString()
                                FirebaseDatabase.getInstance().reference.child("Dinner")
                                    .child(pushKey)
                                    .updateChildren(map)
                            }
